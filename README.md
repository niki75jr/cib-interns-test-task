...
База данных расположена в docker-контейнере, как и сам сервер (в двух раздельных, конечно же, "один процесс - один контейнер") и управляются при помощи docker-compose. Данные для базы данных генерируются в sql-скрипте (~501 строка).
#### Запросы
URL: http://80.249.151.176:8080/
1) GET ``/all ``  выдает список всех записей
